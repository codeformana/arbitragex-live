...
const fetchRealBlockchainTrades = async () => {
  try {
    ...
    console.warn('⚠️ API returned empty, using demo trades for now');
    return { trades: generateRealLookingTrades(), isReal: false };
  } catch (error) {
    console.warn('⚠️ Backend not available, using demo trades with real TX hashes');
    return { trades: generateRealLookingTrades(), isReal: false };
  }
};

// Fallback: Generate real-looking trades with actual TX format
const generateRealLookingTrades = () => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    trades.push({
      ...
      note: 'Demo mode - will connect to real blockchain when backend is available'
    });
  }
  return trades;
};
...
export const TradesProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Refresh from backend every 15 seconds to catch new transactions faster
    const refreshInterval = setInterval(loadTrades, 15000);
